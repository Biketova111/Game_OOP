...
public class View {
    ...
    private static String getChar(int x, int y){
        String out = "| ";
        for (Unit Intelligence: Main.allTeam) {
            if (Intelligence.getCoords().get(0) == x && Intelligence.getCoords().get(1) == y){
                if (!Intelligence.isAlive) {
                    out = "|" + (AnsiColors.ANSI_RED + Intelligence.toString().charAt(0) + AnsiColors.ANSI_RESET);
                    break;
                }
                if (Main.team1.contains(Intelligence)) out = "|" + (AnsiColors.ANSI_GREEN + Intelligence.toString().charAt(0) + AnsiColors.ANSI_RESET);
                if (Main.team2.contains(Intelligence)) out = "|" + (AnsiColors.ANSI_BLUE + Intelligence.toString().charAt(0) + AnsiColors.ANSI_RESET);
                break;
            }
        }
        return out;
